...
function Toggle({ on, onChange }) {
  return (
    <div onClick={() => onChange(!on)} style={{ width:36, height:20, borderRadius:10, background: on ? '#1DB954' : 'rgba(0,0,0,0.15)', cursor:'pointer', position:'relative', transition:'background 0.2s', flexShrink:0 }}>
...
export default function App() {
  ...
  // playlistSettings: { [id]: { visibleInLibrary: bool, useAsAiSource: bool } }
  ...
  // Visible playlists = those with visibleInLibrary !== false (default on)
  const visiblePlaylists=playlists.filter(p=>playlistSettings[p.id]?.visibleInLibrary!==false)
  // AI source playlists = those with useAsAiSource === true
  const aiSourcePlaylists=playlists.filter(p=>playlistSettings[p.id]?.useAsAiSource===true)
  ...
  async function loadPlaylists(){
    ...
    try{
      ...
      setPlaylists(items)
      // Default all to visible, none as AI source
      const defaults={}
      items.forEach(p=>{defaults[p.id]={visibleInLibrary:true,useAsAiSource:false}})
      setPlaylistSettings(defaults)
  ...
  async function loadPlaylistTracks(pl){
    ...
    try{
      ...
      setPlaylistTracksCache(c=>({...c,[pl.id]:items}))
    }finally{setLoading(l=>({...l,pltracks:false}))}
  }

  // Fetch tracks for all AI-source playlists
  async function getAiSourceTracks(){
    const extra=[]
    for(const pl of aiSourcePlaylists.slice(0,5)){
      if(playlistTracksCache[pl.id]){
        extra.push(...playlistTracksCache[pl.id])
      }else{
        try{
          let items=[],url=`https://api.spotify.com/v1/playlists/${pl.id}/tracks?limit=50`
          const d=await spFetch(url)
          items=(d.items||[]).filter(i=>i.track?.name).map(i=>i.track)
          setPlaylistTracksCache(c=>({...c,[pl.id]:items}))
          extra.push(...items)
        }catch{}
      }
  ...
  function updatePlaylistSetting(id,key,val){
    setPlaylistSettings(s=>({...s,[id]:{...s[id],[key]:val}}))
  }
  ...
  async function discover(){
    ...
    try{
      // Gather tracks from AI-source playlists
      setAiStatus('Loading your selected playlists...')
      const extraTracks=await getAiSourceTracks()
      ...
      const res=await fetch('/api/discover',{
        ...
        body:JSON.stringify({
          ...
          extraTracks:extraTracks.slice(0,50),
          token,
        })
      })
  ...
  async function savePlaylist(tracks,name){
  ...
  function NavItem({id,label,icon,onClick}){
    ...
    return(
      <div onClick={onClick||(()=>setPage(id))}
        ...
        {id==='playlists'&&visiblePlaylists.length>0&&<span style={{fontSize:10,background:'#f0f0ee',color:'#aaa',padding:'1px 6px',borderRadius:8}}>{visiblePlaylists.length}</span>}
    ...
    return(
      <>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:18}}>
          {[mood,BASS_LABELS[Math.min(Math.round(bass/20),5)],`${bpmLo}–${bpmHi} bpm`,ENERGY_LABELS[Math.min(Math.round(energy/20),5)],DARK_LABELS[Math.min(Math.round(darkness/20),5)],FAM_LABELS[Math.min(Math.round(familiarity/20),5)],VOX_LABELS[vox]].map(t=>(
    ...
    <div style={{fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',background:'#f5f5f3',minHeight:'100vh'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'200px 1fr',minHeight:'calc(100vh - 49px)'}}>
        ...
        <div style={{background:'#fff',borderRight:'0.5px solid rgba(0,0,0,0.07)',padding:'20px 12px',position:'sticky',top:49,height:'calc(100vh - 49px)',overflowY:'auto'}}>
          ...
          <NavItem id="manage" label="Manage playlists" icon={<svg style={ni} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>}/>
        ...
        <div style={{padding:'28px 32px',maxWidth:900}}>
          ...
          {page==='discover'&&(
            <div>
              ...
              {/* AI SOURCE PLAYLISTS INDICATOR */}
              {aiSourcePlaylists.length>0&&(
                <div style={{background:'#E1F5EE',borderRadius:10,padding:'10px 14px',marginBottom:16,display:'flex',alignItems:'center',gap:10,fontSize:12}}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0F6E56" strokeWidth="2"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
                  <span style={{color:'#085041'}}>AI is also reading <strong>{aiSourcePlaylists.length} playlist{aiSourcePlaylists.length>1?'s':''}</strong>: {aiSourcePlaylists.map(p=>p.name).join(', ')}</span>
                  <span style={{marginLeft:'auto',cursor:'pointer',color:'#0F6E56',fontSize:11}} onClick={()=>setPage('manage')}>manage →</span>
                </div>
              )}
              ...
              {aiTracks.length>0&&(
                <div ref={resultsRef} style={{marginTop:28}}>
                  ...
                  <div style={{display:'flex',gap:10}}>
                    <button onClick={()=>savePlaylist(aiTracks,`wave. ${mood} · ${new Date().toLocaleDateString()}`)}
          ...
          {page==='manage'&&(
            <div>
              <div style={{fontSize:22,fontWeight:500,letterSpacing:-0.4,marginBottom:4}}>Manage playlists</div>
              <div style={{fontSize:12,color:'#999',marginBottom:20}}>Control what shows in your library and what the AI reads.</div>

              {/* LEGEND */}
              <div style={{background:'#fff',border:'0.5px solid rgba(0,0,0,0.08)',borderRadius:12,padding:'14px 16px',marginBottom:16,display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                <div>
                  <div style={{fontSize:12,fontWeight:500,color:'#1a1a1a',marginBottom:4}}>Show in library</div>
                  <div style={{fontSize:12,color:'#999',lineHeight:1.5}}>When off, the playlist is hidden from your library view but still exists on Spotify.</div>
                </div>
                <div>
                  <div style={{fontSize:12,fontWeight:500,color:'#1a1a1a',marginBottom:4}}>Use as AI source</div>
                  <div style={{fontSize:12,color:'#999',lineHeight:1.5}}>When on, the AI reads this playlist to learn your taste and include its tracks in discovery.</div>
                </div>
              </div>

              {/* BULK ACTIONS */}
              <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
                <button onClick={()=>setPlaylistSettings(s=>{const n={...s};playlists.forEach(p=>{n[p.id]={...n[p.id],visibleInLibrary:true}});return n})}
                  style={{padding:'6px 12px',borderRadius:8,fontSize:12,fontFamily:'inherit',cursor:'pointer',border:'0.5px solid rgba(0,0,0,0.12)',background:'#fff',color:'#666'}}>
                  Show all
                </button>
                <button onClick={()=>setPlaylistSettings(s=>{const n={...s};playlists.forEach(p=>{n[p.id]={...n[p.id],visibleInLibrary:false}});return n})}
                  style={{padding:'6px 12px',borderRadius:8,fontSize:12,fontFamily:'inherit',cursor:'pointer',border:'0.5px solid rgba(0,0,0,0.12)',background:'#fff',color:'#666'}}>
                  Hide all
                </button>
                <button onClick={()=>setPlaylistSettings(s=>{const n={...s};playlists.forEach(p=>{n[p.id]={...n[p.id],useAsAiSource:false}});return n})}
                  style={{padding:'6px 12px',borderRadius:8,fontSize:12,fontFamily:'inherit',cursor:'pointer',border:'0.5px solid rgba(0,0,0,0.12)',background:'#fff',color:'#666'}}>
                  Clear AI sources
                </button>
              </div>
              ...
              {loading.playlists?<Loader/>:(
                <div style={{background:'#fff',border:'0.5px solid rgba(0,0,0,0.08)',borderRadius:12,overflow:'hidden'}}>
                  {/* HEADER */}
                  <div style={{display:'grid',gridTemplateColumns:'1fr 110px 110px',gap:12,alignItems:'center',padding:'10px 16px',borderBottom:'0.5px solid rgba(0,0,0,0.07)',background:'#fafaf8'}}>
                    <div style={{fontSize:11,fontWeight:500,color:'#aaa'}}>Playlist</div>
                    <div style={{fontSize:11,fontWeight:500,color:'#aaa',textAlign:'center'}}>Show in library</div>
                    <div style={{fontSize:11,fontWeight:500,color:'#aaa',textAlign:'center'}}>AI source</div>
                  </div>
                  {playlists.map(p=>{
                    const s=playlistSettings[p.id]||{visibleInLibrary:true,useAsAiSource:false}
                    const img=p.images?.[0]?.url
                    return(
                      <div key={p.id} style={{display:'grid',gridTemplateColumns:'1fr 110px 110px',gap:12,alignItems:'center',padding:'12px 16px',borderBottom:'0.5px solid rgba(0,0,0,0.05)'}}>
                        <div style={{display:'flex',alignItems:'center',gap:10,minWidth:0}}>
                          <div style={{width:36,height:36,borderRadius:4,background:'#f0f0ee',overflow:'hidden',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
                            {img?<img src={img} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{opacity:0.2}}><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>}
                          </div>
                          <div style={{minWidth:0}}>
                            <div style={{fontSize:13,fontWeight:500,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',color:s.visibleInLibrary?'#1a1a1a':'#bbb'}}>{p.name}</div>
                            <div style={{fontSize:11,color:'#bbb',marginTop:1}}>{p.tracks?.total||0} tracks</div>
                          </div>
                        </div>
                        <div style={{display:'flex',justifyContent:'center'}}>
                          <Toggle on={s.visibleInLibrary} onChange={v=>updatePlaylistSetting(p.id,'visibleInLibrary',v)}/>
                        </div>
                        <div style={{display:'flex',justifyContent:'center'}}>
                          <Toggle on={s.useAsAiSource} onChange={v=>updatePlaylistSetting(p.id,'useAsAiSource',v)}/>
                        </div>
                      </div>
                    )
                  })}
              ...
              <div style={{display:'flex',gap:10,marginTop:14,flexWrap:'wrap'}}>
                <div style={{background:'#f7f7f5',borderRadius:8,padding:'8px 14px',fontSize:12,color:'#666'}}>
                  <span style={{color:'#aaa'}}>Visible · </span><span style={{fontWeight:500}}>{visiblePlaylists.length} playlists</span>
                </div>
                <div style={{background:'#E1F5EE',borderRadius:8,padding:'8px 14px',fontSize:12,color:'#085041'}}>
                  <span style={{color:'#5DCAA5'}}>AI sources · </span><span style={{fontWeight:500}}>{aiSourcePlaylists.length} playlists</span>
                </div>
              </div>
            </div>
          )}
          ...
          {page==='playlists'&&(
            <div>
              <div style={{display:'flex',alignItems:'baseline',justifyContent:'space-between',marginBottom:4}}>
                <div style={{fontSize:22,fontWeight:500,letterSpacing:-0.4}}>Your playlists</div>
                <span style={{fontSize:12,color:'#1DB954',cursor:'pointer'}} onClick={()=>setPage('manage')}>manage →</span>
              </div>
              <div style={{fontSize:12,color:'#999',marginBottom:20}}>{visiblePlaylists.length} playlists</div>
              {loading.playlists?<Loader/>:(
          ...
          {page==='liked'&&(
            <div>
              ...
              <div style={{display:'flex',gap:8,marginTop:16}}>
                <button onClick={()=>savePlaylist(likedTracks.slice(0,50),'wave. Liked Export')}
          ...
          {page==='weekend'&&(
            <div>
              ...
              <div style={{display:'flex',gap:8,marginTop:14}}>
                <button onClick={()=>savePlaylist(weekendMix,`wave. Weekend Set · ${new Date().toLocaleDateString()}`)}
